...
const EditProviderDialog = ({ setShowEditProviderDialog }) => {
  const dispatch = useDispatch();
  const [value, setValue] = React.useState("");
  const { loading, providerAddSuccess, selectedProvider } = useSelector(
  ...
  React.useEffect(() => {
    if (selectedProvider && Object.keys(selectedProvider)?.length !== 0) {
      ...
      setValue(selectedProvider?.regionsCovered);
    }
  }, [selectedProvider]);
